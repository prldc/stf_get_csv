...
def extract_pdfs(list):  # You can easily extract a list from a .csv with pandas.
    ...
    d = {'nome': ['dummy'], 'acordao': ['dummy'], 'ocr': [False]}
    df = pd.DataFrame(d, columns=['nome', 'acordao', 'ocr'])
    count = 0
    ...
    for pdf in list:
        ...
        if ext == 'pdf':  # Guarantees that the file is a .pdf, otherwise the program will crash when extracting text.
            ...
            df = df.append({'nome': f"{name}", 'acordao': txt, 'ocr': ocr}, ignore_index=True)
            df.to_csv('pdfs_mined_so_far.csv', encoding="utf-8", index=False)  # Saves the files processed so far in case of failure.
            count = count + 1
    ...
    os.remove('pdfs_mined_so_far.csv')  # Deletes the safety copy.
    return df
